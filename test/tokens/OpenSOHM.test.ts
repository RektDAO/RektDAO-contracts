...
const TOTAL_GONS = "10000000000000000";
...
describe("OpenSOHM", () => {
    ...
    let ohm: OpenOHM;
    let sOhm: OpenSOHM;
    let gOhmFake: FakeContract<OpenGOHM>;
    ...
    beforeEach(async () => {
        ...
        gOhmFake = await smock.fake<OpenGOHM>("OpenGOHM");
        ...
        ohm = await new OpenOHM__factory(initializer).deploy(authority.address);
        sOhm = await new OpenSOHM__factory(initializer).deploy();
    });
